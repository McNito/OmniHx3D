...
import com.omnihx3d.states._AlphaState;
import com.omnihx3d.states._DepthCullingState;
import com.omnihx3d.cameras.Camera;
import com.omnihx3d.materials.textures.WebGLTexture;
import com.omnihx3d.materials.textures.VideoTexture;
...
#if (js || purejs)
import com.omnihx3d.audio.AudioEngine;
import js.Browser;
...
@:expose('BABYLON.Engine') class Engine {
	...
	public static var mouseMoveRelative:Array<Dynamic> = [];
	...
	public function new(canvas:Dynamic, antialias:Bool = false, ?options:Dynamic, adaptToDeviceRatio:Bool = false) {		
		...
		var msg:String = "omnihx3d - Cross-Platform 3D Engine | " + Date.now().getFullYear() + " | http://exaphaser.github.io/OmniHx3D/";
	...
	public function createRawCubeTexture(url:String, scene:Scene, size:Int, format:Int, type:Int, noMipmap:Bool = false, callback:ArrayBuffer->Array<ArrayBufferView>):WebGLTexture {
		...
		var width = size;
		var height = width;
		var isPot = (com.omnihx3d.math.Tools.IsExponentOfTwo(width) && com.omnihx3d.math.Tools.IsExponentOfTwo(height));
	...
	public function createDynamicTexture(width:Int, height:Int, generateMipMaps:Bool, samplingMode:Int, forceExponantOfTwo:Bool = true):WebGLTexture {
        ...
        if(forceExponantOfTwo) {
		    width = com.omnihx3d.math.Tools.GetExponentOfTwo(width, this._caps.maxTextureSize);
		    height = com.omnihx3d.math.Tools.GetExponentOfTwo(height, this._caps.maxTextureSize);
        }
	...
	public function createCubeTexture(rootUrl:String, scene:Scene, files:Array<String> = null, noMipmap:Bool = false):WebGLTexture {
		...
		if (isDDS) {
			...
			function generate() {
				var width = com.omnihx3d.math.Tools.GetExponentOfTwo(imgs[0].width, this._caps.maxCubemapTextureSize);
				var height = width;
    ...
    public function prepareTexture(texture:WebGLTexture, scene:Scene, width:Int, height:Int, invertY:Bool, noMipmap:Bool, isCompressed:Bool, processFunction:Int->Int->Void, ?onLoad:Void->Void, samplingMode:Int = Texture.TRILINEAR_SAMPLINGMODE) {
        ...
        var potWidth = com.omnihx3d.math.Tools.GetExponentOfTwo(width, engine.getCaps().maxTextureSize);
        var potHeight = com.omnihx3d.math.Tools.GetExponentOfTwo(height, engine.getCaps().maxTextureSize);
	...
	public function setPointerLock()
	{
		...
		Mouse.hide();
		Mouse.lock = true;
		_renderingCanvas.onMouseMoveRelative.add(function(x:Float, y:Float) {
			for (f in mouseMoveRelative)
			{
				f(x,y);
			}
